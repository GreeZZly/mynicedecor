...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
		...
		$query = $this->db1->select('password, salt')
		                  ->where('id', $id)
		                  ->limit(1)
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			$query = $this->db1->select($this->identity_column)
			...
			$this->trigger_events('extra_where');
			$this->db1->update($this->tables['users'], $data, array($this->identity_column => $identity));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$this->db1->update($this->tables['users'], $data, array('id' => $id));
		}


		$return = $this->db1->affected_rows() == 1;
		if ($return)
		{
	...
	public function clear_forgotten_password_code($code) {
		...
		$this->db1->where('forgotten_password_code', $code);

		if ($this->db1->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
			...
			$this->db1->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$query = $this->db1->select('id, password, salt')
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db1->affected_rows() == 1;
	...
	public function username_check($username = '')
	{
		...
		return $this->db1->where('login', $username)
	...
	public function email_check($email = '')
	{
		...
		return $this->db1->where('email', $email)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function identity_check($identity = '')
	{
		...
		return $this->db1->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db1->update($this->tables['users'], $update, array($this->identity_column => $identity));
...
//		}
		...
		$this->db1->insert($this->tables['users'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db1->select($this->identity_column . ', login, email, id, password, active, last_login, id_registred_company')
		                  ->where('login', $this->db1->escape_str($identity))
		                  ->or_where('email',$this->db1->escape_str($identity))
		                  ->limit(1)
		                  ->get($this->tables['users']);
	...
	function get_attempts_num($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db1->where('ip_address', $ip_address);
			if (strlen($identity) > 0) $this->db1->or_where('login', $identity);

			$qres = $this->db1->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db1->select_max('time');
			$this->db1->where('ip_address', $ip_address);
			if (strlen($identity) > 0) $this->db1->or_where('login', $identity);
			$qres = $this->db1->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $this->db1->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$this->db1->where(array('ip_address' => $ip_address, 'login' => $identity));
			// Purge obsolete login attempts
			$this->db1->or_where('time <', time() - $expire_period, FALSE);

			return $this->db1->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$this->db1->where_in($this->tables['users_groups'].'.'.$this->join['groups'], $groups);
			}
		}
		...
		{
			$this->db1->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$this->db1->limit($this->_ion_limit);
		...
		{
			$this->db1->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->db1->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db1->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		if ($this->db1->where(array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id))->get($this->tables['users_groups'])->num_rows()) return false;

		if ($return = $this->db1->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)))
		{
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			...
			{
				$this->db1->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
		...
		{
			if ($return = $this->db1->delete($this->tables['users_groups'], array($this->join['users'] => (int)$user_id))) {
	...
	public function groups()
	{
		...
		$this->response = $this->db1->get($this->tables['groups']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->db1->where($this->tables['groups'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$this->db1->update($this->tables['users'], $data, array('id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db1->delete($this->tables['users'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db1->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    'old_last_login'       => $user->last_login,
                    'id_registred_company' => $user->id_registred_company      
		);
	...
	public function remember_user($id)
	{
		...
		$this->db1->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db1->select($this->identity_column.', id, login, email, last_login')
	...
	*/
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db1->get_where($this->tables['groups'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		$this->db1->insert($this->tables['groups'], $data);
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db1->get_where($this->tables['groups'], array('name' => $group_name))->row();
		...
		$this->db1->update($this->tables['groups'], $data, array('id' => $group_id));
	...
	**/
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db1->delete($this->tables['users_groups'], array($this->join['groups'] => $group_id));
		// remove the group itself
		$this->db1->delete($this->tables['groups'], array('id' => $group_id));
	...
	{
		$filtered_data = array();
		$columns = $this->db1->list_fields($table);
	...
	protected function _prepare_ip($ip_address) {
		if ($this->db1->platform() === 'postgre' || $this->db1->platform() === 'sqlsrv' || $this->db1->platform() === 'mssql')
		{
			return $ip_address;
		}
		else
    ...
    public function get_user_info($id){
        $query = $this->db1->query("select user.".$this->identity_column.", user.id, user.name, user.surname, user.image_path, user.login, user.email, user.id_registred_company, rc.address as  registred_company, rc.name as company_name
                                   from user
                                   join registred_company rc on rc.id = user.id_registred_company
                                   where user.id = '$id'
                                    limit 1");
    ...
    function setUsersRoles($id,$column,$value){
        $this->db1->query("update users_groups Set '$column' = '$value' where id='$id'");
          //первый вариант
      // $this->db2->update_batch('users_groups',$data,'id');
    }
    // возвращает массив пользователей с правами
    function getUsersRoles(){
        return $this->db1->query("select * from users_groups")->result_array();
    }
    //возвращает обьект пользователя с правами 
    function getUserRoles($id){
       return $this->db1->query("select * from users_groups where user_id ='$id'")->row();
    }
    //обновляет last_time в таблице user
    function updateActivity($email){
        $time = time();
        $this->db1->query("Update user set last_time='$time' where email='$email'");
    }
    //вернет массив пользователей, если не задан id, если задан то вернет пользователя с таким id
     function getUserArray($id_registred_company,$id=null){
     	$sql = " SELECT concat(c.surname,' ',c.name,' ',ifnull(c.second_name,'')) as fullname,email,login, gender, image_path,c.id_passport,c.id_work_place, c.id_bank_details, c.id_contact_info,c.address_id, c.*, a.*,p.*,b.*,co.*,w.*, c.id as id  FROM user c
        ...
        where c.id_registred_company='$id_registred_company'";
        if (isset($id))
        	$sql.= " and c.id ='$id'";
        return $this->db1->query($sql)->result_array();
    ...
    function updateUser($id, $array){
        ...
        //валидация емайл и логин
            if ( $this->email_check($contact['email'])) //$this->identity_column == 'email' &&
            {
                $this->set_error('account_creation_duplicate_email');
                return FALSE;
            }
            elseif ($this->username_check($contact['login'])) //$this->identity_column == 'login' &&
            {
                $this->set_error('account_creation_duplicate_username');
                return FALSE;
            }
            ...
            $this->db1->where('id', $id)->set($contact)->update('user');

            }
            else{
                $ip_address = $this->_prepare_ip($this->input->ip_address());
                $salt       = $this->store_salt ? $this->salt() : FALSE;
                $password   = $this->hash_password(rand(2399999,32228999), $salt);
                ...
                //filter out any data passed that doesnt have a matching column in the users table
                //and merge the set user data and the additional data
                $user_data = array_merge( $contact, $data);

                $user_data['id_contact_info']    = $this->returnInsertedId('id_contact_info' ,'contact_info'     , $contact_info     , $contact);
                $user_data['address_id']         = $this->returnInsertedId('address_id'      ,'address'          , $address          , $contact);
                $user_data['id_bank_details']    = $this->returnInsertedId('id_bank_details' ,'banking_details'  , $bank             , $contact);
                $user_data['id_work_place']      = $this->returnInsertedId('id_work_place'   ,'work_place'       , $work_place       , $contact);
                $user_data['id_passport']        = $this->returnInsertedId('id_passport'     ,'passport'         , $passport         , $contact);
                $this->db1->set($user_data)->insert('user');
                $id = $this->db1->insert_id();
                //add to default group if not already set
                $default_group = $this->where('name', $this->config->item('default_group', 'ion_auth'))->group()->row();
                if ((isset($default_group->id)  && !isset($groups)) || (empty($groups) && !in_array($default_group->id, $groups)))
    ...
    function into_arraY($mass, $array){
        ...
        foreach ($mass as $value) {
            $temp[$value] = (!empty($array[$value])) ? $array[$value] : null;
        }
        return $temp;
    }
    function returnInsertedId($id, $table, $data, $target){
        if($data){
            if(!empty($target[$id])){
                ...
                $this->db1->set($data)->insert($table);
                return $this->db1->insert_id();
            }
        }
    }
    public function create_new_password($identity)
    {
        if (empty($identity))
        {
            $this->trigger_events(array('post_forgotten_password', 'post_forgotten_password_unsuccessful'));
        ...
        if ($return)
            $this->trigger_events(array('post_forgotten_password', 'post_create_password_successful'));
        else
            $this->trigger_events(array('post_forgotten_password', 'post_create_password_unsuccessful'));

        return $return;
    }
    public function create_new_password_complete($code, $salt=FALSE)
    {
        $this->trigger_events('pre_forgotten_password_complete');
