...
class Ion_auth
{
    ...
    public function get_user_info(){
        $id = $this->get_user_id();
       return $this->ion_auth_model->get_user_info($id);
    }
    ...
    public function createUser($id,$values){
        $id = $this->ion_auth_model->updateUser($id,$values);
...
//            return FALSE;
//        }
        if(!$this->ion_auth_model->create_new_password($values['email'])){
            $this->set_error('error create password');
            return FALSE;
        }

        $identity        = $this->config->item('identity', 'ion_auth');
        $user = $this->where($identity, $values['email'])->users()->row();
       // var_dump($user);
        $email = $values['email'];
        $data = array(
            'identity'   => $user->{$identity},
            'id'         => $user->id,
            'email'      => $email,
            'activation' => $user->forgotten_password_code,
        );
        if(!$this->config->item('use_ci_email', 'ion_auth'))
        {
            $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
        ...
        {
            $message = $this->load->view($this->config->item('email_templates', 'ion_auth').'create_password', $data, true);
